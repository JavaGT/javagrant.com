...
async function processAllFeeds() {
    const feeds = Object.values(JSON.parse(await readFileContent('all.json'))).filter(Boolean);
...
async function downloadAndConvertAudio(audioUrl, filePath) {
    ...
    await fsp.writeFile(`${filePath}.audio`, audioBuffer);
    ...
    await fsp.unlink(`${filePath}.audio`);
}
